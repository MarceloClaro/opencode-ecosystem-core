...
1. `pytest prestacao_contas_nota_dez/tests/` → 16/16 verdes (R597 8 + R597b melhorias 8).
...
├── src/{ocr,diligencia,auditor,executor,verificacao,relatorio,conciliar,cli}.py
├── references/  # regras-comuns, base-documental, fontes-oficiais, manual-2019-ocr, checklist-base.csv, modelo-resposta, proveniencia
├── tests/test_r597_prestacao_contas.py + test_r597b_melhorias_plugin.py
└── exemplos/  # audit_*.json, minuta_*.md, controle_*.csv, relatorio_*.md das 2 diligências
.opencode/skills/prestacao-contas-nota-dez/SKILL.md
```

## Resultado real (validação)

- Diligência 2016: 67 itens de checklist (pós-dedup de 83 brutos), minuta + relatório gerados.
- Diligência 2018: 75 itens, minuta + relatório gerados.
