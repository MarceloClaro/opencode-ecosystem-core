...
## 8. Atualização v3.0 — Integração do plugin exportado (ciclo R594 / R-205.v3)

**Data**: 2026-09-25 · **Origem**: plugin exportado `medicos/` (`plugin.json` v0.4.0, `gpt-6ceee9ff15bc7f1a4007d43b810f1876`, autor MARCELO CLARO LARANJEIRA), skill `skills/instructions/SKILL.md` versão **3.0-conselho-longitudinal** + 7 referências.

### 8.1 Requisitos integráveis

| ID | Requisito | Critério de aceitação |
|:---|:----------|:----------------------|
| V3-1 | SKILL.md v3.0 no Core | `skills/medico_virtual_supremo/SKILL.md` presente com frontmatter, versão 3.0, seções 8.2–8.5 (conselho multiespecialidades, anamnese longitudinal/reconstrução reversa, opções terapêuticas, integrações MiroFish/OpenCode) e 12/14 (auditoria final, rodapé obrigatório) |
| V3-2 | 7 referências instrucionais | `skills/medico_virtual_supremo/references/*.md` (SKILL_CHATGPT, diagnostico-diferencial, fontes-diagnosticas, conselho-multiespecialidades, anamnese-longitudinal, opcoes-terapeuticas, integracoes-referencias), todas não vazias |
| V3-3 | API programática fail-closed | `list_references()` → 7 nomes; `load_reference(nome)` → conteúdo ou `None` (nunca lança para nome desconhecido; aceita sufixo `.md`) |
| V3-4 | Release v3 exposto sem quebrar R205 | `SKILL_V3_RELEASE == "3.0-conselho-longitudinal"` e `get_version() == "2.0.0"` (compat test_r205 linha 477) |
| V3-5 | `audit.status` fail-closed nas respostas | Toda resposta clínica tem `audit.status ∈ {aprovado, bloqueado, inconclusivo, requer_escalonamento}` derivado de `checks_failed`; emergência → `requer_escalonamento`; recusa de prescrição → `bloqueado` |
| V3-6 | Paridade com manifesto do plugin | `PLUGIN_SOURCE_ID == "gpt-6ceee9..."`, `PLUGIN_SOURCE_VERSION == "0.4.0"`, `PLUGIN_SOURCE_NOTE` declara que a skill não instala/executa MiroFish-Offline/Neo4j (anti-overclaim R110) |

### 8.2 Anti-overclaim (herdado do R110 + v3.0)

- As verificações SDD/TDD avaliam a consistência da resposta, não são exames nem validação de acurácia clínica.
- `audit.status: aprovado` não significa diagnóstico confirmado, autorização terapêutica nem ausência de risco; `human_review_required: true` permanece.
- A skill adapta padrões de orquestração/memória do MiroFish-Offline/OpenCode; **não instala nem executa** esses motores (Neo4j/Ollama não são executados).
- Rodapé obrigatório v3.0 presente na SKILL.md e `mandatory_footer` na resposta.

### 8.3 Testes

`tests/test_r594_medico_supremo_v3.py` (**23 testes**): SKILL.md v3.0, seções/rodapé/segurança, 7 referências, API fail-closed, release + compatibilidade, paridade do manifesto, smoke de estrutura de resposta v3.0. Regressão R205+R446+R591+R590+R594 = **157 passed + 9 skipped**.

---

**Referências**: SPEC-004 (Transformer), SPEC-005 (Orquestrador), SPEC-934 (MetaBus Integration), `medicina/medicina.txt` (skill base), `skills/medico_virtual_supremo/skill.py` (implementação atual), `medicos/` (plugin exportado gpt-6ceee9ff15bc7f1a4007d43b810f1876 v0.4.0)
