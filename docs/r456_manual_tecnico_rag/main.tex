...
% --- Controle de overfull: permite quebra de palavras longas e espaçamento
%     flexível para evitar caixas estouradas (margens ABNT respeitadas) ---
\emergencystretch=3em
\tolerance=2000
\hbadness=10000

% --- Coluna de tabela com largura fixa e alinhamento à esquerda (evita
%     overfull por justificação em colunas estreitas). Uso: L{0.3\textwidth} ---
...
% Marcador de nível de leitura didático (inline, NÃO usa marginpar para evitar
% colisão/sobreposição nas margens ABNT):
%   \nivel{0} fundamento/intuição | \nivel{1} aplicação | \nivel{2} rigor formal
\renewcommand{\marginpar}[1]{\relax} % desativa marginpar globalmente
\newcommand{\nivel}[1]{%
...
\input{sec_00_capa_rosto}

% --- Corpo (seções numeradas) ---
\input{sec_01_introducao}
\input{sec_02_problema_pesquisa}
\input{sec_03_fundamentacao}
\input{sec_04_estado_atual}
\input{sec_05_proposta_recaman}
\input{sec_06_calculos_specs}
\input{sec_07_implementacoes}
\input{sec_08_mapas_dados}
\input{sec_09_consideracoes}
