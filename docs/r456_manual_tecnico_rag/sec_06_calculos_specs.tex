...
\begin{center}
\small
\begin{tabular}{@{}L{0.30\linewidth}L{0.20\linewidth}L{0.46\linewidth}@{}}
\toprule
\textbf{Estágio} & \textbf{Complexidade} & \textbf{Observação} \\
\midrule
Geração de Recamán ($M$ termos) & $O(M)$ tempo, $O(M)$ memória & valores visitados em \emph{conjunto} \\
Seleção de $K$ itens & $O(K)$ & percurso com \emph{offsets} \\
Resolução de âncoras & $O(N)$ & deduplicação por identidade \\
\textbf{Total} & $O(N)$ & \textbf{não domina} o pipeline \\
\bottomrule
\end{tabular}
\end{center}

\begin{pontochave}
O custo extra do diversificador é linear ($O(N)$) e não altera a ordem de
grandeza do pipeline: resolve-se em memória auxiliar e tempo negligível frente à
codificação de embeddings. É por isso que a proposal \emph{não} degrada a
latência de forma apreciável.
\end{pontochave}
...
implementada.}
\label{tab:spec}
\begin{tabular}{@{}L{0.22\textwidth}L{0.36\textwidth}L{0.38\textwidth}@{}}
\toprule
