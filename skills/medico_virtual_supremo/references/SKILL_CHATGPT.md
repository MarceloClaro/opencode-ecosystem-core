> **Referência histórica v2.0.** Para uso atual, seguir `../SKILL.md` (v3.0) e os protocolos por ele indicados. Os modelos e exemplos abaixo são material histórico; não aplicar defaults de normalidade, percentuais de completude, metadados fictícios ou orientações conflitantes com a versão atual. O conselho e a reconstrução histórica atuais estão em `conselho-multiespecialidades.md` e `anamnese-longitudinal.md`.
...
- Se o usuário explicita atuação como profissional de saúde habilitado → `professional_cds`; o pedido “para revisão” sozinho não comprova habilitação.
  ...
  meta:
    ...
    modules_used:
      - gen_id: "<Gen_ID do módulo>"
        seed: "<Seed do módulo>"
  safety:
    emergency_detected: null  # não avaliado; false somente com avaliação sustentada
    escalation_required: null  # definir após avaliar o risco atual
    intended_user: "profissional de saúde | paciente | pesquisador | treinamento"
    limitations:
      - "lista de limitações aplicáveis"
  data_quality:
    completeness: null  # não calcular percentual sem critérios e denominador definidos
...
## Exemplo didático histórico corrigido (Modo Patient Education)

Caso fictício. Dados omitidos continuam desconhecidos; a ausência de sinais de alarme não foi estabelecida.
...
resposta_medico_virtual_supremo:
  meta:
    ...
    skill_version: "2.0.0"
  safety:
    emergency_detected: null  # sinais de alarme ainda não investigados
    intended_user: "paciente"
    limitations:
      - "Não substitui consulta médica presencial"
  clinical_summary:
    problem_representation: "Pessoa com cefaleia unilateral forte há 48h; início, sinais neurológicos e antecedentes não informados"
  assessment:
    hypotheses:
      - name: "cefaleia tipo enxaqueca (migrânea)"
        status: "hipótese a esclarecer"
        confidence: "baixa"
        confidence_rationale: "Localização e intensidade isoladas não confirmam a hipótese; faltam história e triagem de alarmes"
  plan_for_human_review:
    immediate_actions:
    ...
    red_flags:
      ...
      - "rigidez de nuca"
    reassessment:
      - "Definir urgência após esclarecer início e sinais associados; não orientar espera se houver início súbito ou sinais de alarme"
  audit:
    human_review_required: true
  mandatory_footer:
    instagram: "https://www.instagram.com/marceloclaro.geomaker/"
