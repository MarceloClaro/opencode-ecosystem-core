...
3. **Efeitos fixos:** regressão com efeitos fixos de município e ano via within-transformation dupla (município e ano). Reportam-se erros padrão homocedásticos (transparência) e **erros padrão clusterizados por município** (CRVE com correção de pequena amostra, t com G−1 graus de liberdade).
...
5. **MDES/TOST:** mínimo efeito detectável (poder 80%, α 5%) e teste de equivalência com margem ±0,10 no coeficiente de efeitos fixos, ambos baseados no erro padrão clusterizado (G−1 graus de liberdade).
...
- **H1 seria rejeitada** se o IC95% do bootstrap por cluster incluísse zero ou se a correlação transversal fosse negativa.
- **H2 seria rejeitada** se o coeficiente de efeitos fixos (clusterizado) fosse estatisticamente discernível de zero ao nível de 5% ou se as primeiras diferenças mostrassem correlação |r| ≥ 0,30 com p < 0,05.
- **H3 seria rejeitada** se a maioria dos municípios (≥ 5 de 9) ficasse abaixo da meta pactuada para 2021 ou se o ganho educacional correlacionasse positiva e significativamente com a renda municipal (r > 0,30; p < 0,05).
...
![Figura 1 — Associação em níveis entre IDEB e log do PIB per capita (Sertão de Cratéus, 2013–2023).](outputs/figuras/fig_scatter_niveis_micro.png)

**Tabela 1 — Correlações em níveis por escala (IDEB × log PIB per capita defasado)**

| Escala | n (obs.) | Clusters | r | IC95% bootstrap | p |
|---|---|---|---|---|---|
| Microrregião Sertão de Cratéus | 108 | 9 | 0,49 | [0,34; 0,64] | < 0,001 |
| Ceará | 2.202 | 184 | 0,26 | [0,20; 0,33] | < 0,001 |
| Brasil | 48.695 | 5.360 | 0,46 | [0,44; 0,47] | < 0,001 |
...
| Escala | 1ª diferenças r (p) | n (1ª dif.) | FE β (IC95% cluster) | n (FE) |
|---|---|---|---|---|
...
![Figura 3 — Validação cruzada leave-one-out: r de teste por município retido.](outputs/figuras/fig_loocv_r_teste.png)

**Tabela 3 — Validação cruzada leave-one-out por município (r de teste)**

| Município retido | r treino | r teste | n teste |
|---|---|---|---|
...
Com n municipal de 9 e 108 observações, o mínimo efeito detectável foi de 5,71 pontos de IDEB por unidade de log do PIB (erro clusterizado, G−1 = 8 graus de liberdade; 6,05 com erro homocedástico) — muito acima de qualquer coeficiente plausível. O teste de equivalência (TOST, margem ±0,10) não foi significativo (p = 0,90). Portanto, o correto é declarar **não-detecção**: a amostra não permite afirmar efeito, mas tampouco permite declarar equivalência. Essa é uma limitação central, explicitamente assumida.
...
| Município | IDEB 2007 | IDEB 2025 | Ganho | Meta 2021 | Atingiu meta |
|---|---|---|---|---|---|
...
Os resultados indicam que, no Sertão de Cratéus, a associação entre renda e desempenho escolar é **transversal, não temporal**: municípios com maior PIB per capita tendem a ter IDEB mais alto (r = 0,49; Figura 1), mas a variação temporal da renda não acompanha a variação do desempenho escolar (primeiras diferenças nulas; efeitos fixos não significativos; Figuras 2 e 3). A validação cruzada por município (Figura 4) sugere que a associação transversal é robusta e generalizada, não dependendo de um único município.
...
- Scripts: `scripts/baixar_dados.py`, `scripts/analise_crateus.py`, `scripts/mapa_crateus.py`, `scripts/graficos_crateus.py`.
...
- Figuras: `outputs/figuras/` (scatter de níveis, séries temporais, LOOCV, ganho×renda) e `outputs/mapas/` (renda do responsável, PIB per capita).
...
- Declaração de limites: evidência associativa; n municipal de 9; MDES 5,71 (cluster); não-detecção declarada; IPCE substituído por proxies oficiais.
