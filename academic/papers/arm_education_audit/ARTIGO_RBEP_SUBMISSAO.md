# Educação terciária e trajetórias de renda: evidência associativa de painel com validação cruzada agrupada por país (135 economias, 1960–2023)

**Título:** Educação terciária e trajetórias de renda: evidência associativa de painel com validação cruzada agrupada por país (135 economias, 1960–2023)

**Title:** Tertiary education and income trajectories: associative panel evidence with country-grouped cross-validation (135 economies, 1960–2023)
...
**Palavras-chave:** educação superior; desenvolvimento econômico; governança; dados de painel; validação cruzada por país.
...
**Keywords:** higher education; economic development; governance; panel data; country cross-validation.
...
**Palabras clave:** educación superior; desarrollo económico; gobernanza; datos de panel; validación cruzada por país.
...
Esta revisão é seletiva e orientada por critérios explícitos: foram consultadas as bases Scopus, Web of Science e EconLit, com combinações de termos como "tertiary education", "higher education", "middle-income trap" e "economic growth", priorizando estudos com dados empíricos de painel ou revisões da literatura com definições operacionais explícitas e compatíveis com os dados do Banco Mundial. Não se trata de revisão sistemática com protocolo completo de extração e dupla codificação; o corpus citado é representativo das tradições identificadas, não exaustivo. A seleção final priorizou trabalhos citáveis nas seções seguintes, o que excluiu materiais sem publicação científica periódica ou sem definição operacional verificável.

## 3. Dados e método

### 3.1 Dados

Os dados provêm da API do World Development Indicators e do Worldwide Governance Indicators do Banco Mundial (WORLD BANK, 2024; KAUFMANN; KRAAY; MASTRUZZI, 2011), com download em 13 de agosto de 2026 (verificação contra a API em 14 de agosto de 2026) e cache local imutável (URL, timestamp UTC, status HTTP e hash SHA-256 de cada arquivo em `data/raw_expandido/manifest_expandido.json`). A amostra compreende 135 países, selecionados por critério transparente: países listados no banco oficial do Banco Mundial, com pelo menos 20 observações não nulas de matrícula terciária e de PIB per capita no período. Agregados regionais e territórios sem código ISO3 de país foram excluídos. A seleção não é probabilística, mas é explícita e reprodutível; não se alega representatividade amostral de um universo de países. O detalhamento do processo de amostragem — população, critérios de elegibilidade, motivos de exclusão e cobertura temporal — é apresentado na subseção 4.9.

As variáveis utilizadas são:
- renda: PIB per capita em US$ constantes (NY.GDP.PCAP.KD) e crescimento anual do PIB per capita (NY.GDP.PCAP.KD.ZG);
- educação e inovação: matrícula terciária bruta como proporção do grupo etário correspondente (SE.TER.ENRR), gasto público em educação como proporção do PIB (SE.XPD.TOTL.GD.ZS), dispêndio em pesquisa e desenvolvimento como proporção do PIB (GB.XPD.RSDV.GD.ZS), manufatura como proporção do PIB (NV.IND.MANF.ZS), investimento direto estrangeiro como proporção do PIB (BX.KLT.DINV.WD.GD.ZS) e exportação de alta tecnologia como proporção das exportações (TX.VAL.TECH.MF.ZS);
- estrutura e governança: população urbana como proporção do total (SP.URB.TOTL.IN.ZS) e os seis indicadores do WGI — controle da corrupção, efetividade governamental, estabilidade política, qualidade regulatória, estado de direito e voz e prestação de contas (CC.EST; GE.EST; PV.EST; RQ.EST; RL.EST; VA.EST).
...
Todos os resultados são produzidos pelos scripts `scripts/download_expanded_data.py` e `scripts/analyze_expanded.py`, que leem exclusivamente o cache auditado e escrevem tabelas e um arquivo de proveniência (`outputs/expanded/provenance_expanded.json`). Cada número citado neste artigo tem entrada correspondente nesse arquivo; os números dos canais associativos (seção 4.7) residem em `outputs/channels/provenance_r413.json`. A execução é offline e determinística (sementes fixas).
...
| Variável | Descrição | n | Média | Desvio-padrão |
|---|---|---|---|---|
| NY_GDP_PCAP_KD | PIB per capita (US$ const.) | 7512 | 9.546 | 14.710 |
| SE_TER_ENRR | Matrícula terciária (% bruto) | 4517 | 27,34 | 27,49 |
| SE_XPD_TOTL_GD_ZS | Gasto público educacional (% PIB) | 3807 | 4,21 | 1,72 |
| GB_XPD_RSDV_GD_ZS | P&D (% PIB) | 1987 | 0,88 | 0,96 |
| SP_URB_TOTL_IN_ZS | Urbanização (% pop.) | 8640 | 49,76 | 24,28 |
| WGI_CC | WGI — Controle da corrupção | 3369 | −0,12 | 0,93 |
| WGI_GE | WGI — Efetividade governamental | 3360 | −0,06 | 0,91 |
| WGI_RL | WGI — Estado de direito | 3374 | −0,15 | 0,92 |

*Fonte: elaboração própria a partir de World Development Indicators e Worldwide Governance Indicators (2026). Valores completos em `outputs/expanded/tabela1_descr_expandido.csv`.*

### 4.2 Associações em níveis e em primeiras diferenças

Em níveis, a matrícula terciária correlaciona 0,751 com o log do PIB per capita (n = 4374; Tabela 2), resultado de magnitude elevada, mas típico de comparações entre países em estágios muito diferentes de desenvolvimento. Em primeiras diferenças por país, a correlação cai para 0,146 (n = 4373). A queda reduz a associação a uma ordem de grandeza compatível com associações de magnitude modesta e com a presença de ciclos, indicando que a covariância de níveis é dominada pela heterogeneidade persistente entre economias. A diferença entre as duas correlações é testada formalmente na subseção 4.9, com bootstrap por país (Δ = 0,604; IC95% 0,547 a 0,665; p < 0,001).

| Par | Correlação | n |
|---|---|---|
| Matrícula terciária × log PIB pc (níveis) | 0,751 | 4374 |
| Δ Matrícula terciária × Δ log PIB pc (primeiras diferenças) | 0,146 | 4373 |

*Fonte: elaboração própria. "Δ" indica primeira diferença por país. Valores completos em `outputs/expanded/tabela2_correlacoes_expandido.csv`.*
...
| Estatística (135 folds) | ρ (níveis) |
|---|---|
...
*Fonte: elaboração própria. Valores completos em `outputs/expanded/tabela4_subperiodos_expandido.csv`.*
...
| Associação | ρ parcial | IC95% (bootstrap) | n | Controle |
|---|---|---|---|---|
| Matrícula × saúde (expectativa de vida) | 0,684 | [0,639; 0,728] | 4374 | log-PIB |
| Matrícula × desigualdade (Gini) | −0,184 | [−0,302; −0,042] | 1475 | log-PIB |
| Matrícula × inovação (export. alta tecnologia) | −0,002 | [−0,120; 0,087] | 1566 | log-PIB |
| P&D × inovação (export. alta tecnologia) | 0,250 | [0,103; 0,429] | 1148 | log-PIB |
| Gasto educacional × WGI controle da corrupção | 0,341 | [0,247; 0,423] | 2441 | log-PIB |

*Fonte: elaboração própria. `outputs/channels/provenance_r413.json`; bootstrap por país (500 replicações, semente 42); painel FE com erros padrão clusterizados por país; sem inferência causal.*

### 4.8 O caso brasileiro em perspectiva comparada
...
| Unidade | Matrícula (início) | Matrícula (fim) | Δ matrícula (p.p.) | Cresc. médio anual PIB pc (%) |
|---|---|---|---|---|
...
*Fonte: elaboração própria a partir de `data/raw_expandido/wdi_countries_meta.json` e séries WDI. Valores completos em `outputs/expanded/tabela10_amostragem.csv`.*

A confiabilidade das estatísticas centrais é atestada por intervalos de confiança bootstrap por país (Tabela 11): o ρ de níveis tem IC95% entre 0,697 e 0,809, e o ρ de primeiras diferenças entre 0,118 e 0,177; os intervalos não se sobrepõem, reforçando a separação entre as duas associações. A robustez de semente é verificada com três sementes alternativas (7, 2024 e 123): em todas, o Δ observado permanece 0,604, com IC95% entre 0,543 e 0,665 e p < 0,001.

| Semente | Δ observado | IC95% inferior | IC95% superior | p (Δ ≤ 0) |
|---|---|---|---|---|
...
### A.1 Símbolos estatísticos

| Símbolo | Significado |
|---|---|
| ρ | coeficiente de correlação de Spearman (ordinal), entre −1 e 1 |
| ρ_níveis | correlação em níveis (valores observados de cada ano) |
| ρ_1ªdif | correlação das primeiras diferenças por país |
| Δ | operador de primeira diferença: variação de um ano para o seguinte, calculada por país |
...
| WGI_media | média aritmética dos seis indicadores WGI |

### A.2 Códigos de indicadores (WDI/WGI)

| Código | Indicador | Descrição |
|---|---|---|
| NY.GDP.PCAP.KD | PIB per capita (US$ const.) | produto interno bruto per capita em dólares constantes |
| NY.GDP.PCAP.KD.ZG | Crescimento do PIB pc (%) | crescimento anual do PIB per capita, já em percentual |
| SE.TER.ENRR | Matrícula terciária (% bruto) | matrícula no ensino superior como proporção do grupo etário correspondente |
| SE.XPD.TOTL.GD.ZS | Gasto educacional (% PIB) | gasto público em educação como proporção do PIB |
| GB.XPD.RSDV.GD.ZS | P&D (% PIB) | dispêndio em pesquisa e desenvolvimento como proporção do PIB |
| NV.IND.MANF.ZS | Manufatura (% PIB) | valor adicionado da manufatura como proporção do PIB |
| BX.KLT.DINV.WD.GD.ZS | IDE (% PIB) | investimento direto estrangeiro líquido como proporção do PIB |
| TX.VAL.TECH.MF.ZS | Alta tecnologia (% exp.) | exportações de alta tecnologia como proporção das exportações de manufaturados |
| SP.URB.TOTL.IN.ZS | Urbanização (% pop.) | população urbana como proporção do total |
| SP.DYN.LE00.IN | Expectativa de vida | expectativa de vida ao nascer, em anos |
| SI.POV.GINI | Desigualdade | coeficiente de Gini |
| CC.EST | WGI — controle da corrupção | percepção do exercício do poder público para ganho privado |
| GE.EST | WGI — efetividade governamental | qualidade dos serviços públicos e da formulação de políticas |
| PV.EST | WGI — estabilidade política | probabilidade percebida de instabilidade ou violência política |
| RQ.EST | WGI — qualidade regulatória | capacidade de formular e aplicar regulações adequadas |
| RL.EST | WGI — estado de direito | confiança nas regras da sociedade e na aplicação de contratos |
| VA.EST | WGI — voz e prestação de contas | participação dos cidadãos e liberdade de expressão |
