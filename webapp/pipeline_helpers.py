...
def _adapt_deep_research_raw(raw: Dict[str, Any], question: str) -> Dict[str, Any]:
    ...
    for citation in latest.get("citations", []):
        if isinstance(citation, dict):
            knowledge_sources.append(
                citation.get("entity") or citation.get("source")
                or citation.get("type") or "citation"
            )
    return {
        "question": question,
        "answer": answer,
...
def _adapt_peer_review_raw(raw: Dict[str, Any], title: str) -> Dict[str, Any]:
    repair_plan = raw.get("repair_plan", [])
    verification_agenda = [
        item.get("action", "") for item in repair_plan if item.get("action")
    ]
    meta_review = (
        f"Score geral: {raw.get('overall_score', 0)} | "
        f"Rastreabilidade: {raw.get('traceability', 0)} | "
...
def _adapt_revision_raw(raw: Dict[str, Any], manuscript: str) -> Dict[str, Any]:
    ...
    for revision in reversed(revisions):
        ...
        if proposal.get("revised_text"):
            ...
            break
    changes_applied = sum(
        1 for revision in revisions if revision.get("status") == "applied"
    )
    return {
        "revised_manuscript": revised_manuscript,
        "rebuttal_letter": raw.get("rebuttal_letter", ""),
...
def _adapt_composer_raw(raw: Dict[str, Any], venue: str) -> Dict[str, Any]:
    ...
    flat_consistency = {
        key: value for key, value in consistency.items()
...
_HISTORICAL_PIPELINE_HELPERS = {
    "run_evosci": run_evosci,
    "run_deep_research": run_deep_research,
    "run_peer_review": run_peer_review,
    "run_manuscript_revision": run_manuscript_revision,
    "compose_paper": compose_paper,
}


def _legacy_helpers_are_overridden() -> bool:
    """Detecta substituições explícitas dos adaptadores históricos.

    R107 expõe os cinco adaptadores como pontos de composição. A comparação
    por identidade mantém esse seam compatível com monkeypatches e wrappers,
    sem depender de nomes de testes ou de introspecção sobre mocks.
    """
    return any(
        globals()[name] is not helper
        for name, helper in _HISTORICAL_PIPELINE_HELPERS.items()
    )


def _run_legacy_academic_pipeline(seed_domain: str, max_rounds: int,
                                  venue: str, verbose: bool) -> Dict[str, Any]:
    """Executa o encadeamento R107 pelos helpers públicos da webapp."""
    import time
    ...
    timeline["R101_evosci"] = round(time.time() - t0, 1)

    best_solution = evosci.get("best_solution", {})
    question = best_solution.get("content", seed_domain)
    t1 = time.time()
    ...
    timeline["R102_deep"] = round(time.time() - t1, 1)
    ...
    timeline["R103_review"] = round(time.time() - t2, 1)
    ...
    timeline["R104d_revision"] = round(time.time() - t3, 1)
    ...
    timeline["R105_composer"] = round(time.time() - t4, 1)
    timeline["total"] = round(time.time() - started_at, 1)
...
def _run_orchestrated_academic_pipeline(seed_domain: str, max_rounds: int,
                                        venue: str) -> Dict[str, Any]:
    """Adapta o resultado estruturado do pipeline R108 do orquestrador."""
    from marceloclaro.orchestrator import MarceloClaroOrchestrator

    orchestrator = MarceloClaroOrchestrator(auto_load_agents=False)
    result = orchestrator.scientific_discovery_pipeline(
        seed_domain=seed_domain,
        max_rounds=max_rounds,
        venue=venue,
        strict_gates=True,
    ...
    timeline = {
        "R101_evosci": raw_timeline.get("r101", 0.0),
        "R102_deep": raw_timeline.get("r102", 0.0),
        "R103_review": raw_timeline.get("r103", 0.0),
        "R104d_revision": raw_timeline.get("r104d", 0.0),
        "R105_composer": raw_timeline.get("r105", 0.0),
        "total": raw_timeline.get("total", 0.0),
    }
    ...
    return {
        "pipeline_result": result.get("status", "error"),
        "seed_domain": seed_domain,
        "venue": venue,
        "timeline": timeline,
        "evosci": evosci,
...
def run_full_academic_pipeline(seed_domain: str, max_rounds: int = 3,
                               venue: str = "abnt",
                               verbose: bool = False) -> Dict[str, Any]:
    """Executa o pipeline preservando os contratos R107 e R108.

    Em operação normal, o pipeline unificado do MarceloClaro é a fonte de
    verdade R108. Quando um consumidor substitui explicitamente qualquer
    helper histórico, o encadeamento R107 é usado para que os cinco pontos de
    composição continuem observáveis e o payload da UI permaneça estruturado.
    """
    if _legacy_helpers_are_overridden():
        return _run_legacy_academic_pipeline(
            seed_domain=seed_domain,
            max_rounds=max_rounds,
            venue=venue,
            verbose=verbose,
    ...
    return _run_orchestrated_academic_pipeline(
        seed_domain=seed_domain,
        max_rounds=max_rounds,
        venue=venue,
    )
