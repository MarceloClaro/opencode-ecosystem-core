...
# Espessura estimada por página (mm), para papel offset 75g.
MM_PER_PAGE = 0.0722
...
class CoverDesigner:
    ...
    def _determine_style(self, title: str, content_sample: str) -> str:
        """Determina o estilo por palavras inteiras, sem falsos matches."""
    ...
    def _tikz_art(self, style_key: str, palette: Dict[str, str]) -> str:
        """Gera arte vetorial TikZ full-bleed, específica para cada estilo.

        As coordenadas ficam em um ``scope`` ancorado no canto inferior
        esquerdo da página A4 para que o overlay cubra a folha inteira.
        """
        ...
        head = (
            ...
            f"  \\definecolor{{cPrim}}{{HTML}}{{{primary}}}\n"
            f"  \\definecolor{{cSec}}{{HTML}}{{{secondary}}}\n"
            f"  \\definecolor{{cAcc}}{{HTML}}{{{accent}}}\n"
    ...
    def _spine_mm(self, page_count: int) -> float:
        """Estima, em milímetros, a largura da lombada."""
        return round(max(3.0, page_count * MM_PER_PAGE), 1)

    @staticmethod
    def _ink_for(bg_hex: str) -> str:
        """Escolhe tinta branca ou preta conforme a luminância do fundo."""
        red, green, blue = (int(bg_hex[index:index + 2], 16) for index in (0, 2, 4))
    ...
    def _spine_tex(self, title: str, author: str, palette: Dict[str, str],
                   spine_mm: float) -> str:
        """Gera a lombada vertical dimensionada pela contagem de páginas."""
        return (
    ...
    def _ficha_catalografica(self, title: str, author: str, subtitle: str,
        ...
        """Gera ficha catalográfica no modelo CIP brasileiro."""
    ...
    def design_cover(self, title: str, author: str, content_sample: str,
        ...
        contracapa com arte vetorial TikZ real, lombada dimensionada pela
        contagem de páginas e ficha catalográfica CIP.
        """
