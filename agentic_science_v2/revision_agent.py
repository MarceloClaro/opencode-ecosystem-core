...
class OrchestratorRevision:
    ...
    def run(self, review_package: Dict[str, Any],
        ...
        """Pipeline completo com rollback automático e erro estruturado."""
        try:
            return self._run(review_package, manuscript)
        except Exception as exc:
            logger.exception("Falha no pipeline de revisão de manuscrito: %s", exc)
            return {
                "status": "error",
    ...
    def _run(self, review_package: Dict[str, Any],
        ...
        # 5. Verify — reverte todas as propostas se a integridade estrutural falhar.
        integrity = self.diff_engine.verify_integrity()
        if not integrity["intact"]:
            logger.warning(
                "Integridade do manuscrito comprometida (%s); revertendo mudanças.",
                integrity["issues"],
            )
            reverted = 0
