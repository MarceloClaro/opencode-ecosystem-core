---
name: 36_agente_exportacao_latex_pdf
description: "Converter o manuscrito consolidado em formatos de submissão profissional: **LaTeX** (com classe adequada ao periódico), **PDF** compilado e **DOCX**."
version: '1.0.0'
skills:
- id: 36-agente-exportacao-latex
  name: 36 Agente Exportacao Latex Pdf
  description: >-
    Executa tarefas especializadas de 36 agente exportacao latex pdf conforme protocolo SDD/TDD.
  tags: [36, agente, exportacao]
  examples: [Execute o pipeline MASWOS para este tópico, Pesquise literatura acadêmica sobre este tema]
tags: [academic, agente, exportacao, latex, maswos-agent]
...
> Conteúdo migrado de `criador-artigo/agents/36_agente_exportacao_latex_pdf.md` (SPEC-935-R380): o card anterior era um stub com descrição placeholder e corpo apontando para um caminho externo inexistente neste checkout. Seções de referência a arquivos não migrados (`references/*.md`, `templates/*.md` do projeto de origem) foram omitidas — não fabricadas.
