...
description: "Montar automaticamente, a partir dos fragmentos aprovados (`manuscrito_secoes/00` a `07`), um **documento completo, contínuo e pronto para submissão/defesa** em múltiplos formatos."
version: '1.0.0'
skills:
...
> Conteúdo migrado de `criador-artigo/agents/38_agente_montagem_entrega_final.md` (SPEC-935-R380): o card anterior era um stub com descrição placeholder e corpo apontando para um caminho externo inexistente neste checkout. Seções de referência a arquivos não migrados (`references/*.md`, `templates/*.md` do projeto de origem) foram omitidas — não fabricadas.
