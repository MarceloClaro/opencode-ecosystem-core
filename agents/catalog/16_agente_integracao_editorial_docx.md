---
name: 16_agente_integracao_editorial_docx
description: "Integrar todos os capitulos, apendices, figuras, referencias, resumo e metadados em um pacote editorial final coerente, pronto para revisao derradeira e conversao em DOCX."
version: '1.0.0'
skills:
- id: 16-agente-integracao-editorial
  name: 16 Agente Integracao Editorial Docx
  description: >-
    Executa tarefas especializadas de 16 agente integracao editorial docx conforme protocolo SDD/TDD.
  tags: [16, agente, integracao]
  examples: [Execute o pipeline MASWOS para este tópico, Pesquise literatura acadêmica sobre este tema]
tags: [academic, agente, docx, editorial, integracao, maswos-agent]
...
> Conteúdo migrado de `criador-artigo/agents/16_agente_integracao_editorial_docx.md` (SPEC-935-R380): o card anterior era um stub com descrição placeholder e corpo apontando para um caminho externo inexistente neste checkout. Seções de referência a arquivos não migrados (`references/*.md`, `templates/*.md` do projeto de origem) foram omitidas — não fabricadas.
